    ...
    def _ensure_db_and_load(self) -> None:
        ...
        with self._lock:
            ...
            with self._get_connection() as conn:
                ...
                # Auto-bootstrap from clinical_lexicon.json if database is fresh
                cursor = conn.cursor()
                cursor.execute("SELECT COUNT(*) FROM clinical_lexicon;")
                count = cursor.fetchone()[0]
                json_seed = self.db_path.parent / "clinical_lexicon.json"
                if count == 0 and json_seed.exists():
                    try:
                        data = json.loads(json_seed.read_text(encoding="utf-8"))
                        now = int(time.time())
                        records = [
                            (
                                r["raw_term"].strip().lower(),
                                r["canonical_term"].strip().lower(),
                                r.get("category", "general").strip().lower(),
                                r.get("language", "en").strip().lower(),
                                1 if r.get("prevent_merge", False) else 0,
                                r.get("source", "bootstrap_json"),
                                now,
                                now
                            )
                            for r in data
                        ...
                        conn.executemany("""
                            INSERT OR IGNORE INTO clinical_lexicon (
                                raw_term, canonical_term, category, language, prevent_merge, source, created_at, updated_at
                            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?);
                        """, records)
                        conn.commit()
                        logger.info(f"Auto-bootstrapped {len(records)} terms from {json_seed.name}")
                    except Exception as e:
                        logger.warning(f"Could not auto-bootstrap from {json_seed}: {e}")
...
def main():
    """AI-Native CLI Interface for Clinical Lexicon & Normalizer."""
    import argparse
    parser = argparse.ArgumentParser(description="MedMate Clinical Lexicon & Normalizer CLI")
    parser.add_argument("--stats", action="store_true", help="Print lexicon summary statistics as JSON")
    parser.add_argument("--normalize", type=str, help="Normalize a medical query string")
    parser.add_argument("--search", type=str, help="Search for raw or canonical terms in lexicon")
    parser.add_argument("--export", action="store_true", help="Export entire lexicon to clinical_lexicon.json")
    parser.add_argument("--vacuum", action="store_true", help="Defragment and optimize SQLite database")
    parser.add_argument("--db-path", type=str, default=None, help="Custom database path")

    args = parser.parse_args()
    normalizer = ClinicalNormalizer(db_path=args.db_path)

    if args.stats:
        print(json.dumps(normalizer.get_stats(), ensure_ascii=False, indent=2))
    elif args.normalize:
        norm = normalizer.normalize(args.normalize)
        print(f"Input:      {args.normalize}")
        print(f"Normalized: {norm}")
    elif args.search:
        results = normalizer.search(args.search)
        print(json.dumps(results, ensure_ascii=False, indent=2))
    elif args.export:
        target = normalizer.export_to_json()
        print(f"Exported {normalizer.get_stats()['total_terms']} terms to {target}")
    elif args.vacuum:
        normalizer.vacuum()
        print(f"Vacuum completed for {normalizer.db_path}")
    else:
        parser.print_help()
